...
class DNSNameUnderscoreInTRD(base.LintInterface):
    def Initialize(self):
        return 0
        ...
        try:
            if len(c.subject.get_attributes_for_oid(NameOID.COMMON_NAME)) and not fqdn.CommonNameIsIP(c):
                for name in c.subject.get_attributes_for_oid(NameOID.COMMON_NAME):
                    if "_" in tldextract.extract(name.value).subdomain :
            ...
            for dns in c.extensions.get_extension_for_oid(ExtensionOID.SUBJECT_ALTERNATIVE_NAME).value.get_values_for_type(x509.DNSName):
                if "_" in tldextract.extract(dns).subdomain:
                    return base.LintResult(base.LintStatus.Warn)
...
def init():
    base.RegisterLint(base.Lint("w_dnsname_underscore_in_trd","DNSName should not have an underscore in labels left of the ETLD+1","BRs: 7.1.4.2",base.LintSource.CABFBaselineRequirements,Time.RFC5280Date,DNSNameUnderscoreInTRD()))