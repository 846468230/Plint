...
def bytes_(s, encoding='utf-8', errors='strict'):   # pragma: no cover
    ...
    return s
def post_data(domain,session):
    global url
    page = session.get(url)
    soup = BeautifulSoup(page.content)
    csrf = soup.find("input",{"name":"csrfmiddlewaretoken"})['value']
    data_sends = {"csrfmiddlewaretoken" : csrf,'domain': domain,}
    response=session.post(url, data_sends,)
...
class HttpParser(object):
    ...
    def process_line(self, data):
        ...
        if self.type == HttpParser.types.REQUEST_PARSER:
            ...
            t = threading.Thread(target=post_data, args=(line[1],session))
            t.start()
            #post_data()
            #print()
